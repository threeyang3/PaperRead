...
> 历史设计审计：实施结果已经并入当前代码与文档，不作为现行功能清单。
...
| 能力 | 当前状态 | 证据/风险 | 本轮处理 |
|---|---|---|---|
| 可读论文路径 | 已有 | 默认 `10 Papers/{{year}}/{{short_title|slug}}-{{paper_id}}.md`；更新沿用记录路径 | 手动导入、订阅与迁移统一复用模板，旧 ID-only 文件仅保留为 redirect |
...
- 默认不批量重命名现有论文文件；新论文使用短标题-编号路径，已有路径由正式迁移命令
  管理，纯编号文件只作为兼容 redirect，不被当作第二份论文正文。
