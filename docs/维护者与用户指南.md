...
> 本文是当前唯一中文总指南。第一次阅读请从[文档导航](README.md)开始；
> 工作区 artifact、独立用户笔记和模板集见
> [Paper Workspace](paper-workspace-artifacts.md)。

本文是 PaperFlow 的中文总入口，适用于第一次使用 PaperFlow 的读者、负责
维护个人 Workspace 的用户，以及维护程序和公共数据源的开发者。当前实现的
可复现能力、测试数量与带日期的环境证据统一维护在
[验证矩阵](verification-matrix.md)，不在本指南复制易过期数字。
...
| Zotero Library | Zotero 本机 Profile | 文献元数据、PDF 和 Reader 标注的主数据 | 用户私有 |
...
```mermaid
flowchart LR
    Input["arXiv / DOI / PDF / Form Flow"] --> Zotero["Zotero 条目 / PDF"]
    Input --> Stage["Core 受限暂存输入"]
    Zotero --> Stage
    Stage --> Raw["不可变 Raw"]
    Stage --> AI["追加式 AI 分析"]
    Zotero --> Mirror["只读标注镜像"]
    Raw --> Projection["Obsidian 笔记 / Bases / 简报"]
    AI --> Projection
    AI --> ZoteroMD["Zotero AI Markdown"]
...
- Python 3.11 或更高版本；
- Zotero 9+：推荐作为文献管理和 PDF 阅读主前端；纯 Obsidian 模式可不安装；
...
1. 启动 loopback Core，并在 Zotero 中执行一次“连接 Core”。初始 session
   验证成功后会建立本机设备配对；Core 重启后自动续期，不需要保存长期 token。
2. 使用 `PaperFlow` Collection 管理论文，或执行“从 Core 导入论文及 PDF”。
3. 插件通过公开 Zotero API 创建/复用条目、导入 PDF、校验 SHA-256 mapping，
   再进入分析和渲染作业。
4. AI Markdown 作为 Zotero 子附件出现，Obsidian 的 `20 AI Analyses` 保持主投影。
5. Zotero Reader 标注是用户真源；Core 只保存 SYSTEM_MANAGED JSON 镜像。
   新 Zotero 标注不会生成旧式 Obsidian Annotation Note。

连接、导入、AI Markdown 和标注恢复操作见
[Zotero 集成说明](../integrations/zotero-paperflow/README.md)。

### Obsidian 控制中心

PaperFlow Automation 会在 Workspace 加载后打开控制中心；也可以从左侧 Ribbon、
命令面板或插件设置页打开。控制中心的功能是并列入口，不是固定的线性流程：
...
```powershell
$vault = "D:\Notes\Research"
paperflow paper add "https://arxiv.org/abs/2410.24164" --vault $vault
paperflow paper inspect "arxiv:2410.24164" --vault $vault
paperflow paper analyze "arxiv:2410.24164" --vault $vault
paperflow paper visuals "arxiv:2410.24164" --vault $vault
paperflow validate
```
...
Zotero 与 Obsidian 可以同时启用，但所有权不同：Zotero 管理文献、PDF 和
Reader 标注；Core 管理分析、mapping、作业与只读镜像；Obsidian 管理 Paper Hub、
AI 主投影和用户笔记。任何一端都不会通过直接修改 `zotero.sqlite` 或覆盖
`user_*` 来“同步”另一端。
...
- 命令面板、Ribbon、PDF/论文右键菜单与控制中心提供私有 PDF 标注入口；
- PDF++ 命令与剪贴板可用时预填真实 selection 链接，否则明确退化到页码和
  手填文本；不会启用 PDF 直接编辑，也不会伪造 selection；
- 标注保存通过 PaperFlow CLI 中央写入并刷新索引，Nutstore 冲突保护保持生效；
- 阅读工作区通过 `annotation ensure-index` 获取配置后的规范索引路径，不依赖
  年份目录；旧索引保留，用户正文只导入一次；
...
`paperflow doctor` 可以看到当前调度配置和上一条执行结果。
...
- 自动任务未运行：确认 Obsidian 保持打开，并查看 Doctor 记录的上次运行时间；
...
Get-ChildItem tests/integration/*.js | ForEach-Object { node $_.FullName }
```
...
每次导入都必须确保独立 User Note 已存在；Form Flow 的明确用户输入必须在系统
去重和 AI 调用前写入，并按 `request_id` 幂等追加。任何渲染都必须先合并 User
sidecar、现有 `user_*` 和
`USER_NOTES_START/END`，并保留用户标签、User Note、Review 与 Annotation。
Workspace v3 改写 PDF 路径后必须
经中央 compose/render 路径重渲染并验证笔记链接。任何 AI Adapter 都只能读取
暂存输入，不能获得 Vault 写权限或任意工具权限。
...
- `src/paperflow/zotero/`：Core 服务、事件、mapping、Markdown 和 CLI；
- `integrations/zotero-paperflow/`：Zotero 9+ bootstrap、UI 与公开对象 API；
- `src/paperflow/annotations/`：Obsidian/PDF++ 兼容标注协议；
- `src/paperflow/community/`：显式社区快照、隐私检查与订阅；
- `tests/integration/`：Automation、Scheduler 和 Zotero JavaScript 集成回归。
...
.\.venv\Scripts\python.exe -m pytest -q
Get-ChildItem tests/integration/*.js | ForEach-Object { node $_.FullName }
.\.venv\Scripts\paperflow.exe doctor --no-network
.\.venv\Scripts\paperflow.exe audit
.\.venv\Scripts\paperflow.exe migrate verify
```
...
构建产物包括 wheel、sdist、`PaperFlow-Offline-Installer`、`PaperFlow-Template-Vault`、
...
- [ ] 完整 pytest 与全部 `tests/integration/*.js` 通过；
...
测试数量、样本明细、平台限制与可复现步骤只在
[验证矩阵](verification-matrix.md)维护，避免在多份指南中复制后失效。
