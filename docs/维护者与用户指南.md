...
`USER_NOTES_START/END`，并保留用户标签。Workspace v3 改写 PDF 路径后必须
...
- 仓库完整回归为 202 项 pytest 与 7 个 JavaScript 集成测试文件通过；
- `ArxivLearn` Workspace v3、Doctor、audit、路径、笔记/PDF 链接和 Obsidian
  交互验收通过；
- 隔离 Zotero 9.0.6 Profile 已用 π0.5 完成 Collection、条目、PDF/SHA-256、
  mapping、分析、Zotero AI Markdown、Obsidian 主投影和 Reader 评论镜像闭环；
- 主 Zotero Profile 未用于写入验收，PaperFlow 仍不直接读取或修改
  `zotero.sqlite`；
- 29 篇代表性样本、视觉资产和 ArXiv-data 往返验收结果保留有效。
