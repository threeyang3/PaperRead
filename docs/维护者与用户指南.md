...
开发时以 `src/paperflow` 为唯一程序源码。Vault 内
`.paperflow/src/paperflow` 是由安全升级事务管理的运行时副本，不应提交到
PaperRead，也不应代替正式源码修改。
...
该仓库同时是 PaperRead 源码工作树和当前验收 Vault，因此维护时要严格区分：
...
node --test tests/integration/test_scheduler_core.js tests/integration/test_automation_plugin_lifecycle.js
.\.paperflow\.venv\Scripts\python.exe -m paperflow.cli doctor --no-network
.\.paperflow\.venv\Scripts\python.exe -m paperflow.cli audit
.\.paperflow\.venv\Scripts\python.exe -m paperflow.cli migrate verify
```
...
构建产物包括 wheel、sdist、Windows x64 便携 ZIP、Schema ZIP、模板 ZIP、
迁移说明和 SHA256 清单。构建后重跑 packaging 测试，并在干净环境安装 wheel
执行 `paperflow --help`。
...
- PaperFlow 1.3.2 + Unreleased；
- 186 项 pytest 和 2 项 Node 集成测试通过；
...
- PaperRead 和 ArXiv-data 的最终 GitHub Actions 均通过；
- Obsidian 1.12.7 中文界面、29 行论文 Base 和内部定时任务通过。
