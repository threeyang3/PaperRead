...
> 工作区 artifact、独立用户笔记和模板集的最新操作见
> [Paper Workspace 文档](paper-workspace-artifacts.md)。
...
2026-07-21 的 PaperFlow 1.5.0 源代码基线已通过 Workspace v3 模拟迁移、
默认 dry-run 的正式备份回滚、Community canonical hash 篡改拒绝、111 项 pytest、
2 项 Node 插件测试和发布物白名单检查。隔离空 Workspace 的 Doctor/audit 已验证
应用、版本契约、配置、路径、SQLite、Schema、模板、Automation 和隐私门禁；
真实 `ArxivLearn` Vault 已完成 32 个 record/PDF copy/note render，但原 1.5.0
遗漏 Derived PDF 路径更新，导致验证拒绝 32/32 笔记。源码回归已修复；真实
Vault 仍需安装新 wheel 后重跑 schema3 repair、verify、Doctor、audit、
PDF/笔记链接和 Obsidian 交互检查。
