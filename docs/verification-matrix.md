# PaperFlow 1.4.0 verification matrix
...
| Migration and data preservation | Pass | `migrate status`, `plan`, and `verify` passed with 29 local migrated Raw, 55 subscription-cache Raw (legacy and separated-source acceptance caches), 29 User, 29 Derived, and 84 AI records. The migration is reversible, preserves legacy records, validates SQLite and links, and keeps raw, AI, user, derived, and subscription-cache layers separate. |
...
| Release packaging | Pass | The final build creates wheel, sdist, curated `PaperFlow-portable-1.4.0.zip`, `PaperFlow-Template-Vault-1.4.0.zip`, schema/template ZIPs, checksums, and migration notes. Positive-whitelist audits and four packaging tests exclude papers, PDFs, user notes, runtime state, caches, logs, databases, browser state, and arbitrary `.obsidian` state. |
| Clean installation | Pass | The 1.4.0 wheel replaced the legacy editable `.paperflow/src` runtime and imports from `.paperflow/.venv/Lib/site-packages`; the plugin no longer injects any Vault source directory into `PYTHONPATH`. |
| Automated regression | Pass | The 93 tests in the authoritative repository `tests/` tree and both Node plugin suites passed, including encoding/title guards, immutable Raw captures, four-layer visual preservation, adaptive images, relationship schemas/rebuild, ChatGPT JSON handling, Feed privacy, source/Vault separation, lifecycle, and scheduler coverage. Runtime environments do not contain a second test source tree. |
...
1. Install the 1.4.0 wheel or use the PaperFlow portable archive.
