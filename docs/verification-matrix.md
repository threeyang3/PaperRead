...
| Automated regression | Pass | The 91 tests in the authoritative repository `tests/` tree and both Node plugin suites passed, including encoding/title guards, immutable Raw captures, four-layer visual preservation, adaptive images, relationship schemas, ChatGPT JSON handling, Feed privacy, lifecycle, and scheduler coverage. The legacy runtime test copy under `.paperflow/tests` is not a second source of truth. |
