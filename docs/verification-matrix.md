...
| Caption-backed paper figures | Pass | Four real local PDFs produced 23 Derived PNGs with category-balanced, architecture-first ranking (6/5/6/6). Diffusion Policy, TacForeSight, NIST IDB, and RoboTTT crops were visually inspected; an NIST two-column false positive and a clipped RoboTTT architecture top edge were found and fixed during QA. All four manifests pass schema, Vault-bound path, source-PDF SHA256, and PNG SHA256 validation. |
...
| Automated regression | Pass | 175 pytest tests plus Obsidian lifecycle and scheduler Node integration suites passed. |
