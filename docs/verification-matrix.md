...
| User-focused Control Center | Source pass; real Vault rerun pending | The lifecycle DOM contains one task shelf with seven current goals: collect, discover, library, reading, annotation, reproduction, and daily review. Analysis and Agent configuration are separate input cards. The real Vault must install this wheel before the new annotation entry can be accepted interactively. |
| PDF visual annotation loop | Source pass; real Vault interaction pending | PDF++ 0.40.31 is optional. The adapter feature-detects `pdf-plus:copy-link-to-selection`, validates the clipboard link against the current versioned PDF, and otherwise opens an explicit page/text fallback. Command palette, ribbon, file menu, reading workflow and Control Center entries use the same bilingual modal. Saves go through `annotation create --apply`, rebuild the private index, keep quote text separate from PDF++ coordinates, and retain version/reanchor data. Direct PDF editing stays disabled. |
...
| Automated regression | Pass | All 113 tests in the authoritative repository `tests/` tree and both Node plugin suites (2/2) pass. The lifecycle suite loads the real plugin-directory `main.js` from an unrelated cwd while an Electron-like loader rejects every relative module request. It covers four unique reading leaves, PDF focus, PDF++ selection success/fallback, annotation CLI allowlisting and the modal UI. Coverage additionally includes Workspace v3 repair, rollback, Community tamper rejection, Feed privacy, lifecycle, and scheduling. |
...
| Runtime plugin health | Source layout repair pass; real Vault layout rerun pending | After the self-contained-entry repair, ArxivLearn reloads the plugin with no console error. Real command execution then opened PDF, Annotation and Community but omitted Review and left the original paper focused. The layout now creates Review from the PDF leaf, awaits PDF reveal and focuses it explicitly; the next fixed wheel must be installed before repeating the four-tab, console, DOM and screenshot checks. |
