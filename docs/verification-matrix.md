...
| Agent provider and model settings | Pass | The Control Center and `ai set-profile` atomically configure profile, Codex/Claude/Mock provider, model, timeout, fallback, reuse, and reanalysis policy. Provider probes found Codex, Claude Code, and deterministic Mock. A real Claude Code 2.1.206 run through the configured Xiaomi endpoint persisted π0, FAST, and π0.5 analyses with explicit `mimo-v2.5` provenance. |
...
| Caption-backed paper figures | Pass | 29 representative embodied-AI/tactile/VLA PDFs produced 166 Derived PNGs (3–6 per paper) with category-balanced, architecture-first ranking. All 29 manifests pass schema, Vault-bound path, source-PDF SHA256, and PNG SHA256 validation. |
...
| Automated regression | Pass | 186 pytest tests passed, including Claude schema compatibility, local reanalysis, immutable Raw preservation, subscription-aware migration verification, Obsidian Doctor startup tolerance, resource-link rejection, visual Bases, Feed privacy, and Windows `core.autocrlf` clone coverage. |
