...
| Automated regression | Pass | All 111 tests in the authoritative repository `tests/` tree and both Node plugin suites (2/2) pass. The lifecycle suite loads the real plugin-directory `main.js` from an unrelated cwd while an Electron-like loader rejects every relative module request. Its layout mock also requires four unique leaves, anchors Review to the PDF leaf, awaits reveal, and verifies PDF focus. Coverage additionally includes Workspace v3 repair, rollback, Community tamper rejection, Feed privacy, lifecycle, and scheduling. |
