...
async function main() {
  ...
  assert.match(requests[3].url, /zotero\/papers\/import$/);
  assert.match(requests[4].url, /zotero\/staging\/arxiv%3A2504\.16054$/);
  assert.equal(requests[4].options.headers["Content-Type"], "application/pdf");
  assert.match(requests[5].url, /zotero\/events$/);
  assert.match(requests[6].url, /analysis\/jobs$/);
  assert.match(requests[7].url, /jobs\?limit=5$/);
  assert.match(requests[8].url, /jobs\/zotero-analysis-abc$/);
  assert.match(requests[9].url, /zotero\/migration\/results$/);
  assert.match(requests[10].url, /community\/publish$/);
  assert.match(requests[11].url, /subscriptions\/inbox\?limit=10&status=pending-confirmation$/);
  assert.match(requests[12].url, /subscriptions\/inbox\/arxiv%3A2504\.16054$/);
  assert.match(requests[13].url, /subscriptions\/inbox\/decision$/);
